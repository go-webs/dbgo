...
dbGame.Table("users").List("title", "name")    // {name: title}
// Chunking Results
...
dbGame.Table("users").Where("votes",">",100).OrWhere(func(query dbgo.SubQuery){
...
// Where Not Clauses
dbGame.Table("products").WhereNot(func(query dbgo.SubQuery){
...
dbGame.Table("users").When(role, func (query dbgo.SubQuery, role string) {
